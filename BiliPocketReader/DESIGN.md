...
3. `animations.js`
4. `comic-reader-images.js`
5. `reader-preferences.js`
6. `reader-screenshot.js`
7. `comic-reader-page-groups.js`
8. `comic-reader-interactions.js`
9. `comic-reader.js`
10. `content-page-info.js`
11. `content-url.js`
12. `dynamic-filter.js`
13. `space-opus-tabs.js`
14. `favorites-text-dialog.js`
15. `settings-popover-ui.js`
16. `favorites-ui.js`
17. `content.js`
...
- 新增持久设置时，先在 `TOOLBOX_SETTINGS` 中加 key，再在 `createDefaultData()` / `normalizeToolboxData()` 中给默认值。
...
- 主类应尽量保留“状态 + 行为 + 渲染入口”，避免继续膨胀事件绑定和分页规则。
...
- `animationMode`：`none` / `smooth` / `fade`。
...
优先改 `comic-reader-interactions.js`。如果新增按钮，需要同时在 `comic-reader.js` 的 DOM 创建和按钮同步方法中接入。
...
```powershell
rg -n "dynamic-controls-ui|dynamicControlsUi|bilibili-fav-controls-panel" BiliPocketReader
git status --short
```
